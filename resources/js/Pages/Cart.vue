    ...
    <StoreLayout>
        <div class="bg-slate-50 min-h-screen py-20 font-sans">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="mb-12">
                    <h1 class="text-4xl font-black text-slate-900 tracking-tight italic uppercase">Shopping <span class="text-[#003366]">Cart</span></h1>
                    <p class="text-slate-500 font-bold mt-2 uppercase tracking-widest text-[10px]">Review your selection before checkout</p>
                </div>
                ...
                <div v-else class="grid grid-cols-1 lg:grid-cols-12 gap-12">
                    <div class="lg:col-span-8">
                        <div v-if="cart.length > 0" class="bg-white shadow-xl shadow-slate-200/50 rounded-3xl border border-slate-100 overflow-hidden">
                            <ul role="list" class="divide-y divide-slate-50">
                                <li v-for="item in cart" :key="item.id" class="flex p-8 group transition-colors hover:bg-slate-50/50">
                                    <div class="flex-shrink-0 w-24 h-24 sm:w-40 sm:h-40 bg-slate-100 rounded-2xl overflow-hidden relative border border-slate-100">
                                        <img v-if="item.product?.image" :src="item.product.image" :alt="item.product.name" class="w-full h-full object-center object-cover group-hover:scale-105 transition-transform duration-500" />
                                        ...
                                        <div class="flex items-start justify-between">
                                            <div>
                                                <h3 class="text-xl font-black text-slate-900 italic uppercase tracking-tight">
                                                    <Link :href="`/products/${item.product?.slug}`" class="hover:text-[#003366] transition-colors">{{ item.product?.name }}</Link>
                                                </h3>
                                                <p class="text-[10px] font-bold text-slate-400 mt-1 uppercase tracking-[0.2em]">{{ item.product?.sku }}</p>
                                            </div>
                                            <p class="text-2xl font-black text-[#003366]">৳{{ parseFloat(item.product?.price || 0).toLocaleString() }}</p>
                                        </div>
                                        <div class="mt-auto flex items-end justify-between">
                                            <div class="flex items-center space-x-4 bg-slate-100 p-1.5 rounded-xl">
                        ...
                        <div v-else class="bg-white shadow-2xl shadow-slate-200/50 rounded-3xl border border-slate-100 p-20 text-center flex flex-col items-center">
                    ...
                    <div class="lg:col-span-4" v-if="cart.length > 0">
                        <div class="bg-[#003366] text-white rounded-3xl p-10 sticky top-24 shadow-2xl shadow-[#003366]/30 overflow-hidden relative">
                            ...
                            <dl class="space-y-6">
                                <div class="flex items-center justify-between">
                                    <dt class="text-[10px] font-black text-slate-300 uppercase tracking-widest">Subtotal</dt>
                                    <dd class="text-lg font-black">৳{{ cartTotal.toLocaleString() }}</dd>
                                </div>
                                <div class="flex items-center justify-between border-t border-white/10 pt-6">
                                    <dt class="flex flex-col">
                                        <span class="text-[10px] font-black text-slate-300 uppercase tracking-widest">Shipping</span>
                                        <span class="text-[8px] font-black text-[#FF6600] mt-1 italic uppercase tracking-widest">Cash on Delivery</span>
                                    </dt>
                                    <dd class="text-[10px] font-black uppercase tracking-widest italic">Calculated next</dd>
                                </div>
                                <div class="flex items-center justify-between border-t border-white/20 pt-8 mt-4">
                                    ...
                                    <dd class="text-3xl font-black text-[#FF6600]">৳{{ cartTotal.toLocaleString() }}</dd>
                                </div>
                            </dl>

                            <div class="mt-12">
                                <Link v-if="$page.props.auth.user" href="/checkout" class="w-full flex items-center justify-center rounded-2xl bg-white px-8 py-5 text-[10px] font-black text-[#003366] shadow-xl hover:bg-[#FF6600] hover:text-white transition-all uppercase tracking-[0.3em]">
                                    Proceed To Checkout
                                </Link>
                                <Link v-else href="/login" class="w-full flex items-center justify-center rounded-2xl bg-[#FF6600] px-8 py-5 text-[10px] font-black text-white shadow-xl hover:bg-orange-600 transition-all uppercase tracking-[0.3em]">
                                    Sign In To Checkout
                                </Link>
                            </div>
                        </div>
