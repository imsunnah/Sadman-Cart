<template>
    <AdminLayout>
        <div class="mb-10 flex items-center justify-between bg-white p-8 rounded-xl shadow-sm border border-slate-100 font-sans">
            <div>
                <h1 class="text-2xl font-bold text-[#003366]">Add New Brand</h1>
                <p class="text-xs text-slate-500 mt-1">Establish a brand name and connect associated categories</p>
            </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden max-w-3xl font-sans">
            <form @submit.prevent="submit" class="p-8 space-y-8">
                <!-- Brand Name -->
                <div>
                    <label class="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">Brand Name</label>
                    <input v-model="form.name" type="text" placeholder="e.g. Al-Noor, Ceylon Naturals" class="w-full px-4 py-3 rounded-lg bg-slate-50 border-none focus:ring-2 focus:ring-[#003366]/10 outline-none transition-all font-bold">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 bg-slate-50 p-6 rounded-2xl border border-slate-100">
                        <label v-for="category in categories" :key="category.id" class="flex items-center space-x-3 cursor-pointer p-3 bg-white rounded-xl shadow-sm border border-slate-100 hover:border-[#003366]/30 transition-all select-none">
                            ...
                            >
                            <span class="text-xs font-bold text-slate-700 uppercase tracking-tight">{{ category.name }}</span>
                        </label>
                    </div>
